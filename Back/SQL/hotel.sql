...
SELECT num_chambre FROM chambres WHERE equipement LIKE '%televiseur%';
...
SELECT AVG(prix/nb_pers) FROM chambres;
SELECT AVG(prix/nb_pers) FROM chambres WHERE equipement LIKE '%televiseur%';
SELECT prix/nb_pers FROM chambres WHERE equipement LIKE '%televiseur%';
...
-- Les numéros des chambres coûtant au minimum 80 Euro ayant un bain et valant au maximum 120 Euro.
SELECT num_chambre FROM chambres WHERE prix BETWEEN 120 AND 300 AND confort LIKE '%jacuzzi%';

-- Les numéros des chambres coûtant au maximum 80 Euro ou ayant un bain et volant au maximum 120 Euro(facultatif).

-- Les Nom, Prénoms et adresses des clients dans le prenom commencent par "D".
SELECT nom, prenom, adresse FROM clients WHERE prenom LIKE 'd%';

-- Le nombre de chambres dont le prix est entre 85 et 120 Euro.
SELECT COUNT(num_chambre) FROM chambres WHERE prix BETWEEN 120 AND 300;
...
SELECT * FROM clients AS c INNER JOIN reservations AS r WHERE   c.nom_client = r.num_client;
